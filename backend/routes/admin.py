...
def move_job_to_printer(job_id):
    ...
    if job['status'] != 'queued':
        return jsonify({'success': False, 'message': 'Cannot move: only queued jobs can be moved to another printer'}), 400
    ...
    if not target or target['status'] != 'active':
        return jsonify({'success': False, 'message': 'Target printer not found or not active'}), 400
